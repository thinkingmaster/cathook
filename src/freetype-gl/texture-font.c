        ...
        FT_Library *library, FT_Face *face)
{
    ...
    if(error) {
        fprintf(stderr, XStr("FT_Error (0x%02x) : %s\n"),
    ...
    if(error) {
        fprintf(stderr, XStr("FT_Error (line %d, code 0x%02x) : %s\n"),
                         ...
                         const char * codepoint )
{
    ...
    if( !codepoint )
    {
        ...
        if ( region.x < 0 )
        {
            fprintf( stderr, XStr("Texture atlas is full (line %d)\n"),  __LINE__ );
            FT_Done_Face( face );
            FT_Done_FreeType( library );
            return NULL;
        }
