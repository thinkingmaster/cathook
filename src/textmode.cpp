...
void EXPOSED_Epic_VACBypass_1337_DoNotSteal_xXx_$1_xXx_MLG() {
	...
	uintptr_t Host_IsSecureServerAllowed_addr = gSignatures.GetEngineSignature(XStr("55 89 E5 83 EC 18 E8 ? ? ? ? 8B 10 C7 44 24 04 ? ? ? ? 89 04 24 FF 52 2C 85 C0 74 11 C6 05 90 43 88 00 00"));
	// +0x21 = allowSecureServers
	logging::Info(XStr("1337 VAC bypass: 0x%08x"), Host_IsSecureServerAllowed_addr);
	...
	logging::Info(XStr("Allow Secure Servers: 0x%08x"), allowSecureServers);
	*allowSecureServers = true;
	logging::Info(XStr("Done..?"));
}

CatCommand fixvac(XStr("fixvac"), XStr("Lemme in to secure servers"), []() {
	*allowSecureServers = true;
});
...
InitRoutine init([]() {
	...
	logging::Info(XStr("[TEXTMODE] Setting up input handling"));
	int flags = fcntl(0, F_GETFL, 0);
	flags |= O_NONBLOCK;
	fcntl(0, F_SETFL, flags);
	logging::Info(XStr("[TEXTMODE] stdin is now non-blocking"));
#endif
